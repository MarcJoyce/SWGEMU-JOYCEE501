...
bool CraftingSessionImplementation::validateSession() {
	...
	if (strongStation != nullptr) {
		...
		if (droidParent != nullptr) {
			if (!droidParent->isInRange(strongCrafter, 8.0f)) {
		...
		} else if (!strongStation->isInRange(strongCrafter, 8.0f)) {
			StringIdChatParameter param("ui", "sui_out_of_range_prose"); // You have gone out of range of %TT, closing interface.
...
void CraftingSessionImplementation::initialAssembly(int clientCounter) {
	...
	int experimentationSkill = crafter->getSkillMod(expskill);
	int forceSkill = crafter->getSkillMod("force_experimentation");
	int deitySkill = crafter->getSkillMod("deity_techno_union");
	experimentationPointsTotal = int((experimentationSkill + forceSkill + deitySkill) / 10);

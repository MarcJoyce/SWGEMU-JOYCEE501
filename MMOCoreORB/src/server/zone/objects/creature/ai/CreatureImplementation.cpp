...
bool CreatureImplementation::canHarvestMe(CreatureObject* player) {
	...
	if(!player->isInRange(_this.getReferenceUnsafeStaticCast(), 32.0f) || !player->hasSkill("outdoors_scout_novice")
...
bool CreatureImplementation::canDroidHarvestMe(CreatureObject* player,CreatureObject* droid) {
	...
	if(!droid->isInRange(_this.getReferenceUnsafeStaticCast(), (32.0f + droid->getTemplateRadius() + getTemplateRadius())) || !player->hasSkill("outdoors_scout_novice")
...
bool CreatureImplementation::canMilkMe(CreatureObject* player) {
	...
	if(!player->isInRange(_this.getReferenceUnsafeStaticCast(), 8.0f) || player->isInCombat() || player->isDead() || player->isIncapacitated() || !(player->hasState(CreatureState::MASKSCENT)))
		return false;

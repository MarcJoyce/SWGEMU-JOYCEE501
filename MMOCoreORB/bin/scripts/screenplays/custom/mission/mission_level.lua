...
function mission_level:start()

end

function mission_level:openWindow(pPlayer)
    if (pPlayer == nil) then
        return
    end
...
function mission_level:showLevels(pPlayer)
    ...
    local sui = SuiListBox.new("mission_level", "levelSelection") -- calls level selection on a SUI window event
...
function  mission_level:levelSelection(pPlayer, pSui, eventIndex, args)
	...
	writeScreenPlayData(pPlayer, "mission_level", "levelChoice", selectedLevel) 

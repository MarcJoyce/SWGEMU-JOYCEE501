...
EncounterPadawan = Encounter:new {
  taskName = "EncounterPadawan",
  -- Encounter properties
  encounterDespawnTime = 2 * 60 * 1000,
  spawnObjectList = {
    { template = "encounter_padawan", minimumDistance = 96, maximumDistance = 128, referencePoint = 0, followPlayer = true, setNotAttackable = false, runOnDespawn = true },
...
function EncounterPadawan:isTheFirstMobOfThePlayer(pMob, pPlayer)
...
function EncounterPadawan:onPlayerKilled(pPlayer, pKiller, nothing)
  if (pPlayer == nil or pKiller == nil) then
    return 0
  end

  Logger:log("Player was killed.", LT_INFO)
  if SpawnMobiles.isFromSpawn(pPlayer, EncounterPadawan.taskName, pKiller) then
    -- spatialChat(pKiller, "")
    createEvent(10 * 1000, "EncounterPadawan", "handleDespawnEvent", pPlayer, "")
...
function EncounterPadawan:onEncounterSpawned(pPlayer, spawnedObjects)
...
function EncounterPadawan:onEncounterInRange(pPlayer, spawnedObjects)
  if (pPlayer == nil or spawnedObjects == nil or spawnedObjects[1] == nil) then
    return
  end
...
function EncounterPadawan:onMobDestroyed(pVictim, pAttacker)
  if (pVictim == nil) then
    return 0
  end
...
function EncounterPadawan:isEncounterFinished(pPlayer)
  if (pPlayer == nil) then
    return false
  end
...
function EncounterPadawan:taskFinish(pPlayer)
  if (pPlayer == nil) then
    return true
  end

  JediEncounters:startStepDelay(pPlayer, 3)
  return true
end

return EncounterPadawan
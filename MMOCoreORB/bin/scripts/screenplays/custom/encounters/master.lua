...
EncounterMaster = Encounter:new {
  taskName = "EncounterMaster",
  -- Encounter properties
  encounterDespawnTime = 2 * 60 * 1000,
  spawnObjectList = {
    { template = "encounter_master", minimumDistance = 96, maximumDistance = 128, referencePoint = 0, followPlayer = true, setNotAttackable = false, runOnDespawn = true },
    ...
    { template = "encounter_padawan", minimumDistance = 2, maximumDistance = 4, referencePoint = 2, followPlayer = true, setNotAttackable = false, runOnDespawn = true }
  },
...
function EncounterMaster:isTheFirstMobOfThePlayer(pMob, pPlayer)
...
function EncounterMaster:onPlayerKilled(pPlayer, pKiller, nothing)
  if (pPlayer == nil or pKiller == nil) then
    return 0
  end

  Logger:log("Player was killed.", LT_INFO)
  if SpawnMobiles.isFromSpawn(pPlayer, EncounterMaster.taskName, pKiller) then
    -- spatialChat(pKiller, "")
    createEvent(10 * 1000, "EncounterMaster", "handleDespawnEvent", pPlayer, "")
...
function EncounterMaster:onEncounterSpawned(pPlayer, spawnedObjects)
...
function EncounterMaster:onEncounterInRange(pPlayer, spawnedObjects)
  if (pPlayer == nil or spawnedObjects == nil or spawnedObjects[1] == nil) then
    return
  end
...
function EncounterMaster:onMobDestroyed(pVictim, pAttacker)
  if (pVictim == nil) then
    return 0
  end
...
function EncounterMaster:isEncounterFinished(pPlayer)
  if (pPlayer == nil) then
    return false
  end
...
function EncounterMaster:taskFinish(pPlayer)
  if (pPlayer == nil) then
    return true
  end

...
EncounterGCWTroopersRebel = Encounter:new {
  taskName = "EncounterGCWTroopersRebel",
  ...
  spawnObjectList = {
    ...
    { template = "fbase_stormtrooper_hard", minimumDistance = 2, maximumDistance = 4, referencePoint = 4, followPlayer = true, setNotAttackable = false, runOnDespawn = true },
  },
...
function EncounterGCWTroopersRebel:isTheFirstMobOfThePlayer(pMob, pPlayer)
...
function EncounterGCWTroopersRebel:onPlayerKilled(pPlayer, pKiller, nothing)
  if (pPlayer == nil or pKiller == nil) then
    return 0
  end

  Logger:log("Player was killed.", LT_INFO)
  if SpawnMobiles.isFromSpawn(pPlayer, EncounterGCWTroopersRebel.taskName, pKiller) then
    -- spatialChat(pKiller, "")
    createEvent(10 * 1000, "EncounterGCWTroopersRebel", "handleDespawnEvent", pPlayer, "")
...
function EncounterGCWTroopersRebel:onEncounterSpawned(pPlayer, spawnedObjects)
...
function EncounterGCWTroopersRebel:onEncounterInRange(pPlayer, spawnedObjects)
  if (pPlayer == nil or spawnedObjects == nil or spawnedObjects[1] == nil) then
    return
  end
...
function EncounterGCWTroopersRebel:onMobDestroyed(pVictim, pAttacker)
  if (pVictim == nil) then
    return 0
  end
...
function EncounterGCWTroopersRebel:isEncounterFinished(pPlayer)
  if (pPlayer == nil) then
    return false
  end
...
function EncounterGCWTroopersRebel:taskFinish(pPlayer)
  if (pPlayer == nil) then
    return true
  end
...
return EncounterGCWTroopersRebel
...
EncounterKnight = Encounter:new {
  taskName = "EncounterKnight",
...
function EncounterKnight:isTheFirstMobOfThePlayer(pMob, pPlayer)
...
function EncounterKnight:onPlayerKilled(pPlayer, pKiller, nothing)
  if (pPlayer == nil or pKiller == nil) then
    return 0
  end

  Logger:log("Player was killed.", LT_INFO)
  if SpawnMobiles.isFromSpawn(pPlayer, EncounterKnight.taskName, pKiller) then
    -- spatialChat(pKiller, "")
    createEvent(10 * 1000, "EncounterKnight", "handleDespawnEvent", pPlayer, "")
...
function EncounterKnight:onEncounterSpawned(pPlayer, spawnedObjects)
...
function EncounterKnight:onEncounterInRange(pPlayer, spawnedObjects)
  if (pPlayer == nil or spawnedObjects == nil or spawnedObjects[1] == nil) then
    return
  end
...
function EncounterKnight:onMobDestroyed(pVictim, pAttacker)
  if (pVictim == nil) then
    return 0
  end
...
function EncounterKnight:isEncounterFinished(pPlayer)
  if (pPlayer == nil) then
    return false
  end
...
function EncounterKnight:taskFinish(pPlayer)
  if (pPlayer == nil) then
    return true
  end

  JediEncounters:startStepDelay(pPlayer, 4)
  return true
end

return EncounterKnight